                    ...
                    <div className="w-full max-w-2xl bg-deep-sea border border-terminal-green/50 rounded-xl p-6 max-h-[80vh] overflow-y-auto relative animate-in fade-in zoom-in-95 duration-300">
                        ...
                        {gems.length === 0 ? (
                            ...
                            <div className="space-y-4">
                                {gems.map((gem, idx) => (
                                    <div key={idx} className="p-4 bg-black/50 border border-t-yellow-500/30 border-b-blue-500/30 border-x-transparent rounded-lg relative overflow-hidden group hover:border-terminal-green/50 transition-colors">
                                        <div className="absolute top-2 right-2 text-xs text-yellow-500/50 group-hover:text-yellow-400">
                                            GEM #{idx + 1}
                                        </div>
                                        <p className="text-gray-200 font-serif leading-relaxed pr-6">
                                            "{gem}"
                                        </p>
                                    </div>
                                ))}
